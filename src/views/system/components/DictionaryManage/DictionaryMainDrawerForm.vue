<template>
  <section class="dictionary-main-drawer-form-container">
    <base-form
      ref="form"
      :grid="grid"
      :watch="watch"
...
// 导入表单配置
import FormConfig from './DictionaryMainDrawerForm.js'

export default {
  name: 'DictionaryMainDrawerForm',
  mixins: [FormConfig],
  props: {
    disabled: {
      type: Boolean,
  ...
  methods: {
    ...
    doDeleteData (records) {
      return dictionaryApi.deleteBaseClass(requestBuilder('delete', records)).then(res => {
        if (res.code !== '0000') {
    ...
    doUpdateData (record) {
      return this.doValidateData(record).then(records => {
        if (!this.spinning) {
          ...
          const promise =
            this.action === 'insert'
              ? dictionaryApi.addBaseClass(requestBuilder(this.action, records[0]))
              : dictionaryApi.modifyBaseClass(requestBuilder(this.action, records[0]))
