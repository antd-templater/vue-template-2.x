<template>
  <section class="dictionary-child-drawer-form-container">
    <s-form
      ref="form"
      :grid="grid"
      :watch="watch"
...
// 导入表单配置
import FormConfig from './DictionaryChildDrawerForm.js'

export default {
  name: 'DictionaryChildDrawerForm',
  mixins: [FormConfig],
  props: {
    disabled: {
      type: Boolean,
  ...
  methods: {
    ...
    doDeleteData (records) {
      return dictionaryApi.deleteBaseCode(requestBuilder('delete', records)).then(res => {
        if (res.code !== '0000') {
    ...
    doUpdateData (record) {
      return this.doValidateData(record).then(records => {
        if (!this.spinning) {
          ...
          const promise =
            this.action === 'insert'
              ? dictionaryApi.addBaseCode(requestBuilder(this.action, records[0]))
              : dictionaryApi.modifyBaseCode(requestBuilder(this.action, records[0]))
