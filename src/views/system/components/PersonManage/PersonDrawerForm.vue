...
// 导入表单配置
import FormConfig from './PersonDrawerForm.js'

export default {
  name: 'PersonDrawerForm',
  mixins: [FormConfig],
  props: {
    queryOptions: {
      type: Object,
  ...
  methods: {
    ...
    doDeleteData (records) {
      return personApi.deletePersonInfo(requestBuilder('delete', records)).then(res => {
        if (res.code !== '0000') {
    ...
    doUpdateData (record) {
      return this.doValidateData(record).then(records => {
        if (!this.spinning) {
          ...
          const promise =
            this.action === 'insert'
              ? personApi.addPersonInfo(requestBuilder(this.action, records[0]))
              : personApi.modifyPersonInfo(requestBuilder(this.action, records[0]))
