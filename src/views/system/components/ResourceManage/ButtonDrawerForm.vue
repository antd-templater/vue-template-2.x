<template>
  <section class="button-drawer-form-container">
    <s-form
      ref="form"
      :grid="grid"
      :watch="watch"
...
// 导入表单配置
import FormConfig from './ButtonDrawerForm.js'

export default {
  name: 'ButtonDrawerForm',
  mixins: [FormConfig],
  props: {
    queryOptions: {
      type: Object,
    ...
    doUpdateData (record) {
      return this.doValidateData(record).then(records => {
        if (!this.spinning) {
          ...
          const promise =
            this.action === 'insert'
              ? resourceApi.addResourceInfo(requestBuilder(this.action, records[0]))
              : resourceApi.modifyResourceInfo(requestBuilder(this.action, records[0]))
