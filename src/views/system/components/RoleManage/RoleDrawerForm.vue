<template>
  <section class="role-drawer-form-container">
    <s-form
      ref="form"
      :grid="grid"
      :watch="watch"
...
// 导入表单配置
import FormConfig from './RoleDrawerForm.js'
...
export default {
  ...
  methods: {
    ...
    doDeleteData (records) {
      return roleApi.deleteRoleInfo(requestBuilder('delete', records)).then(res => {
        if (res.code !== '0000') {
    ...
    doUpdateData (record) {
      return this.doValidateData(record).then(records => {
        if (!this.spinning) {
          ...
          const promise =
            this.action === 'insert'
              ? roleApi.addRoleInfo(requestBuilder(this.action, records[0]))
              : roleApi.modifyRoleInfo(requestBuilder(this.action, records[0]))
