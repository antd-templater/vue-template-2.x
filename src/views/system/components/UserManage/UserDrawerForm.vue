...
export default {
  ...
  methods: {
    ...
    doDeleteData (records) {
      return userApi.deleteUserInfo(requestBuilder('delete', records)).then(res => {
        if (res.code !== '0000') {
      ...
      return new Promise((resolve, reject) => {
        this.validateFields(errors => {
          ...
          resolve([
            {
              ...data,
              roleId: data.roleId ? data.roleId.split(',') : []
            }
          ])
        })
      })
    ...
    doUpdateData (record) {
      return this.doValidateData(record).then(records => {
        if (!this.spinning) {
          ...
          const promise =
            this.action === 'insert'
              ? userApi.addUserInfo(requestBuilder(this.action, records[0]))
              : userApi.modifyUserInfo(requestBuilder(this.action, records[0]))
